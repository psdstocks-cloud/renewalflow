...
function artly_handle_sync_request(WP_REST_Request $request)
{
  ...
  if (!empty($updated_after)) {
    global $wpdb;
    $date_str = gmdate('Y-m-d H:i:s', strtotime($updated_after));
    $user_ids = array();

    // 1. Users created after date
    ...
    // 2. Users with orders modified after date
    // WC_Order_Query is heavy, use SQL for speed
    ...
    // When using 'include', 'offset' works on the included list
    // But we need to be careful if user_ids is huge.
