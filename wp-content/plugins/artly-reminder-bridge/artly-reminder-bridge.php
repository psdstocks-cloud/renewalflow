...
function artly_reminder_bridge_post_events( array $events ): ?array {
  ...
  if ( is_wp_error( $response ) ) {
    artly_reminder_bridge_log( 'Error syncing events: ' . $response->get_error_message() );
    return null;
  }
...
function artly_reminder_bridge_handle_post(): ?string {
  ...
  if ( isset( $_POST['artly_sync_now'] ) ) {
    ...
    return __( 'Manual sync triggered.', 'artly-reminder-bridge' );
  }
  ...
  <div class="wrap">
    ...
    <form method="post" style="max-width: 700px; margin-top: 20px;">
      ...
      <p class="submit">
        ...
        <button type="submit" name="artly_sync_now" value="1" class="button"><?php esc_html_e( 'Sync now', 'artly-reminder-bridge' ); ?></button>
      </p>
    </form>
  </div>
  <?php
